...
import {
    httpDrugOutputQuery,
    httpDrugOutputInsert,
    httpDrugOutputUpdate,
    httpDrugOutputDelete
} from '@/api/drugOutput.js'
...
const rules = reactive({
    ...
    total: [{ required: true, message: '请输入出库数量' },],
...
function handleDialogConfirm() {
    formRef.value.validate((valid, fields) => {
        if (valid) {
            ...
            if (!form.value.id) {
                // 新增
                httpDrugOutputInsert(form.value)
                    .then(res => {
                        loading.close()
                        handleDialogCancel()
                    })
                ...
                // 编辑
                httpDrugOutputUpdate(form.value)
                    .then(res => {
                        loading.close()
                        handleDialogCancel()
                    })
...
function handleDrugInfo(drugName) {
    httpDrugCatelogueQuery({ drugName: drugName })
        ...
        .catch(err => {
            console.log("🚀 ~ file: DrugOutput.vue:195 ~ handleDrugInfo ~ err:", err)
        })
}
